...
def create_joint_accounts():
    ...
    # limiting the maximum and minimum degrees possible
    degrees = np.clip(degrees,1,10)

    for i in range(num_users):
        # Possible connections that the node can have
        possible_connections = list(set(range(num_users)) - {i} - node_connections[i])

        # Connections that're left to have
        connections_to_add = degrees[i] - len(node_connections[i])

        # If it is possible to make a connection and degree is not fulfilled
        if connections_to_add > 0 and possible_connections:
            # Randomly choosing the connections to be added for sufficing the degree
...
def fire_transactions_analyse_success_rate(n_transactions,node_connections):
    ...
    plt.savefig("transactions_success_rates.png")
    plt.show()

register_users()
node_connections = create_joint_accounts()
assign_funds(node_connections)
fire_transactions_analyse_success_rate(10000,node_connections)